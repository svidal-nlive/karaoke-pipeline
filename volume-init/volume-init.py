...
DIRS = [
    "/input", "/queue", "/logs", "/metadata", "/output", "/organized",
    "/stems", "/cookies", "/chromium_config", "/profile"
]
...
def main():
    ...
    uname = pwd.getpwuid(puid).pw_name if puid in [u.pw_uid for u in pwd.getpwall()] else str(puid)
    gname = grp.getgrgid(pgid).gr_name if pgid in [g.gr_gid for g in grp.getgrall()] else str(pgid)
    log(f"🔑 [volume-init] Target ownership: {puid}:{pgid} ({uname}:{gname})\n")
    table = []
    error_count = 0
    for path in DIRS:
        status = ensure_dir(path, puid, pgid)
        table.append((path, status))
        if status.startswith("ERROR"): error_count += 1
    log("\n📝 Summary:")
